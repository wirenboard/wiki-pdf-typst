...
- Загружает только те PDF, содержимое которых действительно изменилось: свежий файл сравнивается по SHA-1 с тем, что уже лежит на вике
- Игнорирует пуши в файлы, не влияющие на отрисовку: README, .gitignore, вспомогательные скрипты, а также `wiki_publish.py` и `lib/wiki_api.py` — выборка страниц и заливка не меняют содержимое PDF

### Один прогон за раз

Workflow объявляет группу `concurrency`, поэтому прогоны не идут параллельно, а становятся в очередь. Без этого несколько мержей подряд поднимают несколько прогонов `--force` сразу, и они сталкиваются на вике: `Could not acquire lock`, совпадающие пути в архиве, а прогон на старом коде дописывает поверх того, что уже опубликовал прогон на новом.

`cancel-in-progress` намеренно `false`: полная перегенерация идёт около 15 минут, а расписание тикает каждые 10, так что с отменой она никогда не доходила бы до конца.

### Воспроизводимость

Дата в PDF (`/CreationDate`) берётся из даты правки статьи, а не из системных часов. Поэтому одна и та же ревизия страницы всегда компилируется в один и тот же файл. Без этого каждый прогон `--force` давал бы 94 файла, отличающихся от хранящихся только меткой времени, и вика сохраняла бы их как новые версии — около 900 МиБ на каждый пуш.
...
- На обложку берётся вводная иллюстрация статьи — первый `<figure>` (на старых вики `div.thumb`) вне галереи; из текста она удаляется, чтобы не дублироваться. Изображение галереи попадает на обложку только на странице, где других изображений нет, и тогда остаётся в галерее: повтор дешевле, чем потерянный рисунок с подписью
...
- На обложке размещается вводная иллюстрация статьи (правила выбора — в разделе «Обработка изображений»)
