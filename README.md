...
### Шаблон `Wbincludes:Pdf`

Сам шаблон живёт на вики — [Wbincludes:Pdf](https://wiki.wirenboard.com/wiki/Wbincludes:Pdf). Он размечен под перевод и содержит собственную документацию, поэтому правится там, а не из репозитория.

Шаблон строит имя файла собственной формулой, и она должна совпадать с `sanitize_filename()` в `wiki_publish.py`. MediaWiki заменяет на дефис символы `:`, `/` и `\` (`$wgIllegalFileChars`). Если формулы разойдутся, бот загрузит PDF под одним именем, а шаблон сошлётся на другое — на странице появится красная ссылка при полностью исправном файле.
